...
training_config = configs.BlipFullTrainingConfig()
...
logger = Logger('blip_full', logger_config.level, logger_config.path)
...
for e in range(1, training_config.epochs + 1):
    ...
    for i in range(0, len(prompts), training_config.batch_size):
        ...
        for text_prompt in prompts[i:i + training_config.batch_size]:
            ...
            logits = utils.generate_logits_seq_blip(
                model=model,
